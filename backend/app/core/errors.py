...
class AppError(Exception):
    """Domain error rendered with the API-001 envelope (FND-008). `code` must exist in 02_ERROR_CODES."""
...
def _field_path(location: tuple[Any, ...]) -> str:
    ...
    path = ""
    for part in (str(p) for p in location if p not in ("body", "query", "path", "header")):
...
def register_error_handlers(app: FastAPI) -> None:
    ...
    async def _http_error(request: Request, exc: StarletteHTTPException) -> JSONResponse:
        code = _HTTP_STATUS_CODES.get(exc.status_code, "internal_error" if exc.status_code >= 500 else "bad_request")
